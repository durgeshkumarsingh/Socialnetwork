...
$m=$_SESSION['m'];
$sql=("SELECT * FROM  messages where User_Id='$m' order by timestamp desc");
...
echo "<a href='vote.php?a=2 && b=".$fetch['id']."'><img src='images/buttonsfeat2.jpg' ></a></div>".nl2br($fetch['content'])."</div><div style='margin-left:55px;'><form name='dispatches' action='comment.php?n=".$fetch['id']."' method='post'>
...
while($fetch1=mysql_fetch_array($result1))
{
	...
	if($fetch['id']==$fetch1['msgid']){
		echo "<div style='margin-left:60px;'><a href='otherprofile.php?m=".$fetch1['User_Id']."'><img src='upload/".$fetch1['Image']."' height='30' width='30'></div><div style='margin-left:94px;margin-top:-30px;'><strong>".$fetch1['Name'].":  </strong></a><a href='cmntdelete.php?cid=".$fetch1['c_id']." && cmntr=".$fetch1['User_Id']." && msgr=".$fetch1['msgposterid']."'>Delete</a><br>".$fetch1['comment']."<hr></div>";
